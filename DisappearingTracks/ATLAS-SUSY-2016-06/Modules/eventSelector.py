...
class EventSelector(object):
	...
	#Store the probability of select an event with up to 2 tracklets
	self.TA_TE_TP = np.array([0.]*len(self.tau_array))
	...
	TATETP = [0.]*len(self.tau_array)
        for i,tauns in enumerate(self.tau_array): # loop sobre taus
            ...
            ntracklets[i] = weight_Track #guarda el numero de tracklets por cada tau
	...
	MCevent = MCvalues[0]+MCvalues[1]
        ...
        weight_Track = weight_1 - weight_2*TP
	...
	Ntrack_obs = 9.0
	    ...
	    ## Probability of select up to 2 tracklets
	    total_TA_TE_TP = self.TA_TE_TP[i]
